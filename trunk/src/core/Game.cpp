...
Game::Game() :
	panel_(ControlPanel::GetInstance()), message_(GET_BITMAP_FONT("retro")),
...
void Game::Init() {
	...
	mini_map_->SetPosition((APP_WIDTH - mini_map_->GetWidth()) / 2, (APP_HEIGHT
	- mini_map_->GetHeight()) / 2);
...
void Game::ChangeZoneContainer(ZoneContainer::MapName map_name) {
	...
	if (!zone_container_.SetActiveZone(next_zone_cds_.x, next_zone_cds_.y,
			false)) {
...
void Game::Teleport(const Zone::Teleporter& tp) {
	ZoneContainer::MapName map_name =
			(ZoneContainer::MapName) tp.zone_container;
	if (map_name != zone_container_.GetName()) {
		...
		if (!zone_container_.SetActiveZone(tp.zone_coords.x, tp.zone_coords.y)) {
			OutputE << GAME_S << "Mauvaise coordonnees pour SetActiveZone"
					<< lEnd;
			abort();
		}
	}
