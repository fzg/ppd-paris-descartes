...
class TileMap:
    ...
    def __init__(self, file, tileX, tileY):
        ...
        print "Creating unique tiles dictionnary:"
        self.FullTileMap = {}
        ...
        while self.G.nodes() != []:
            ...
            self.G.delete_nodes_from(TileMap.values())
        print "Done !"
        print "Creating Tileset image file:"
    ...
    def addEdge(self, s, t, dir):
        ...
        """
	try:
       		values = [ value for value in self.G.get_edge(s, t) if (dir * value) > 0 ]
	except networkx.exception.NetworkXError:
		values = False
        if values:
            self.G.delete_edge(s, t, values[0])    # increase the value by 1
    ...
    def growTileMap(self, TileMap, G, posX, posY, curV):
        ...
        #       2 <-> down
        #       3 <-> left
        LL = [e for e in G.in_edges(curV)  if e[2] > 0]
        LU = [e for e in G.in_edges(curV)  if e[2] < 0]
        LR = [e for e in G.out_edges(curV) if e[2] > 0]
        LD = [e for e in G.out_edges(curV) if e[2] < 0]
        Edges = [LU, LR, LD, LL]
